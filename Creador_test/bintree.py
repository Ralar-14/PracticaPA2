...
class BinTree:
    ...
    def set_left(self,left):
        """
        Pre: left is a BinTree and the BinTree (self) is not empty
    ...
    def set_right(self,right):
        """
        Pre: right is a BinTree and the BinTree (self) is not empty
    ...
    def leaf(self):
        """
        returns True if the BinTree is a leaf, False if not. The BinTree (self) is not empty
        """
        assert(not self.empty())
    ...
    def __str__(self):
        if not self.empty():
            x = self.get_root()
            return ' ' + str(x) + str(self.get_left()) + str(self.get_right())
        else:
            return ' 0'
...
# creamos clase que lea preordre? 